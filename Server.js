...
let AGENTS = []; // optional persistent agents list; can also send agents per request
...
You are running GLOBAL PLUG TRADE v2.1.1.
...
- safety-first
- scrap-based
- position sizing milestone system
...
Sizing context:
- base size ladder for SYSTEM_1:
  cycleProfit < 150 => 2
  cycleProfit >= 150 => 4
  cycleProfit >= 300 => 6
- drawdown throttle may reduce size externally

Behavior:
- tighter system
- smaller milestone profile
- cleaner entries
...
You are running GLOBAL PLUG TRADE v4.1.
...
- high-gear tightening when size >= 100
- safety-first
- more advanced scaling system
...
Behavior:
- ladder/expansion system
- stricter at higher sizes
- do not output explanations
- return ONLY one valid decision token
`;
...
function getActiveSystem() {
  return LOOP_STATE.activeSystem;
}
...
async function getTicker(pair) {
  const res = await fetch(`${KRAKEN}/0/public/Ticker?pair=${pair}`);
...
async function runAgent(model, pair, price, systemId, systemSize) {
  const systemPrompt = systemId === "SYSTEM_1" ? SYSTEM_1_PROMPT : SYSTEM_2_PROMPT;
...
`;
  ...
  return valid.includes(raw) ? raw : "BLOCK_SAFETY";
}
...
app.post("/api/system", (req, res) => {
  ...
  if (!["SYSTEM_1", "SYSTEM_2"].includes(system)) {
    return res.status(400).json({
      success: false,
...
app.post("/api/engine/execute", async (req, res) => {
  try {
    ...
    const agents = Array.isArray(req.body.agents) && req.body.agents.length
      ? req.body.agents
      : AGENTS;
    ...
    for (const agent of agents) {
      ...
      const decision = await runAgent(agent.model, pair, price, systemId, systemSize);
...
app.listen(PORT, () => {
  console.log(`🚀 GLOBAL PLUG FULL LOOP BACKEND LIVE ON ${PORT}`);
});
